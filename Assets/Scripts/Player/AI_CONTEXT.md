...
一句话接入：场景里放 `First Third Person Controller.prefab`，运行后 `SimplePlayerController` 调度 `1/2/3/4/5` 切换各视角功能；仅在第一人称下，`6` 才会开关 `PlayerDetailInspectView` 详情查看。
...
- `PlayerFixedCameraView`：固定视角选择和播放组件，按 `4` 显示屏幕底部图标栏，点击图标后把玩家相机切到配置的固定视角点，禁用玩家移动，只允许小范围旋转。
...
7. 漫游结束时，如果 `结束后回到第一人称` 为 true，控制器切回 `PlayerViewMode.FirstPerson`。
8. 所有玩家和设置面板按键都通过 `RuntimeInput` 读取，避免项目只启用 Input System 时触发 `UnityEngine.Input` 异常。
